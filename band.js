...
class PartitionedBand {
  ...
  isVertical() {
    return mustBeVertical(this.ord)
  }
...
function makeBandClass(rule) {
  const vertical = mustBeVertical(BAND_COUNT)
...
function postProcessGallery(imgObjs) {
...
function filterSafe(IMG_OBJS) {
  return IMG_OBJS.filter(it => (it.ord % 10 == 0 || it.ord < 10))
}

function filterUnsafe(IMG_OBJS) {
  const unsafeSet = new Set(
    IMG_OBJS.map(it => it.ord)
            .filter(ord => ord % 10 == 0)
            .flatMap(tens => IMG_OBJS.some(itt => itt.ord == tens + 1) ? tens + 1 : tens)
  )
